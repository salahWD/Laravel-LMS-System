...
@section('styles')
  @vite(['resources/csslibs/slick.css', 'resources/csslibs/simple-line-icons.css', 'resources/csslibs/style.css', 'resources/css/custom.css', 'resources/css/editor.css'])
@endsection
    ...
    <div class="container-xl">
        ...
        <div class="post post-single">
          ...
          <div class="post-header">
            ...
            <ul class="meta list-inline mb-0">
              <li class="list-inline-item"><a href="#"><img load="lazy" src="{{ $article->user->image_url() }}"
                    class="rounded-circle author" alt="author" />{{ $article->user->fullname() }}</a></li>
              @if ($article->category_id)
                <li class="list-inline-item"><a href="#">{{ $article->category?->title }}</a></li>
              @endif
            ...
            <div class="row d-flex align-items-center">
              ...
              <div class="col-md-5 col-12">
                <div class="about-author padding-30 rounded">
                  <div class="thumb">
                    <img load="lazy" src="{{ $article->user->image_url() }}" class="rounded-circle"
      ...
      <div class="comments bordered padding-30 rounded">
        ...
        <form id="comment-form" class="comment-form" method="post"
          action="{{ route('comment_article', $article->id) }}">
            ...
            <div class="column col-md-12">
              ...
              <div class="form-group">
                <textarea name="comment" id="comment" class="form-control" rows="3"
                  placeholder="{{ __('leave a comment') }}..." required>{{ old('comment') }}</textarea>
          ...
          <button type="submit" name="submit" id="submit" value="Submit"
            class="btn btn-default">{{ __('Submit') }}</button><!-- Submit Button -->
        ...
        <ul class="comments" id="comments-container">
          @foreach ($comments as $comment)
            ...
            <li class="comment rounded">
              <div class="thumb">
                <img load="lazy" class="rounded-circle" style="max-width: 80px;"
                  src="{{ $comment->user->image_url() }}" alt="{{ $comment->user->fullname() }}" />
              </div>
              <div class="details">
                <h4 class="name"><a href="#">{{ $comment->user->fullname() }}</a></h4>
                <span class="date">{{ $comment->created_at->format('Y-m-d h:ma') }}</span>
                <p>{{ $comment->text }}</p>
                <button id="reply-{{ $comment->id }}-btn" data-id="{{ $comment->id }}"
                  class="comment-reply-btn btn btn-default btn-sm">{{ __('Reply') }}</button>
              </div>
            </li>
          @endforeach
        </ul>
