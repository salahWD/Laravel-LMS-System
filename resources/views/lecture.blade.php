        ...
        <div class="container">
            <div class="row">
                <sidebar class="col-md-3">
                    ...
                    <div class="accordion my-3 mb-4" id="courseContentAccordion">
                        <div class="accordion-item">
                            <h2 class="accordion-header" id="course-content">
                                <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                                    data-bs-target="#contentCollapse" aria-expanded="false" aria-controls="contentCollapse">
                                    Course Content
                                </button>
                            </h2>
                            <div id="contentCollapse" class="accordion-collapse collapse" aria-labelledby="course-content"
                                    ...
                                    <ul class="list-group mt-1">
                                        @if ($all_items)
                                            @foreach ($all_items as $i => $item)
                                                <li
                                                    class="list-group-item @if ($item->is_current) list-group-item-primary @endif">
                                                    @if ($item->is_open)
                                                        <a
                                                            href="@if ($item->is_lecture()) {{ route('lecture_show', $item->itemable_id) }} @else {{ route('test_show', $item->itemable_id) }} @endif">
                                                    @endif
                                                    <div class="d-flex w-100 justify-content-between align-items-center">
                                                        <span
                                                            class="badge @if ($item->is_last_watched || $item->is_current) bg-primary @elseif($item->is_open) bg-success @else bg-dark @endif p-2 py-1 me-2">
                                                        ...
                                                        @if ($item->itemable->duration != null)
                                                            <span
                                                                class="badge bg-dark d-inline-block ms-auto">{{ $item->itemable->duration }}</span>
                                                        @endif
                                                    </div>
                                                    @if ($item->is_open)
                                                        </a>
                ...
                <section class="lecture col-md-9">
                    ...
                    @if ($current_lecture->is_yt_video())
                        <div class="ratio ratio-16x9" id="videoPreviewHolder"
                            data-video="{{ $current_lecture->video_url() }}">
                                ...
                                <div class="options">
                                    <div class="actions">
                                        ...
                                        <div id="volume" class="volume low option">
                                            ...
                                            <div class="input">
                                                <input min="0" value="50" max="100" type="range"
                                                    id="volume-level">
                    ...
                    <div class="text-center">
                        @if (!empty($current_lecture->next_item()))
                            @if ($current_lecture->next_item()->is_lecture())
                                <a id="next-lecture"
                                    href="{{ route('lecture_show', $current_lecture->next_item()->itemable_id) }}"
                                    class="btn btn-lg border btn-outline-success mt-4 m-auto">{{ __('next lecture') }}</a>
                            @else
                                <a id="next-lecture"
                                    href="{{ route('test_show', $current_lecture->next_item()->itemable_id) }}"
                                    class="btn btn-lg border btn-outline-success mt-4 m-auto">{{ __('take a test') }}</a>
                            @endif
                        @else
                            <a id="next-lecture" href="{{ route('courses_show') }}"
                                class="btn btn-lg border btn-outline-success mt-4 m-auto">{{ __('congratulations, you have finished the course') }}</a>
                        @endif
                    </div>
