...
@section('content')
  ...
  <div class="container mt-4">
    <form method="POST" id="product-form" action="{{ isset($product->id) ? route("product_edit", $product->id) : route("product_create") }}" enctype="multipart/form-data">
      @csrf
      <div class="row">
          ...
          <div class="card card-body border-0 shadow mb-4">
            ...
            <div class="mb-3" id="type-section">
              ...
              <div class="form-check">
                <input class="form-check-input @error('product_type') is-invalid @enderror" type="radio" name="product_type" id="dropshipping" value="1" @if($product->type != null && $product->type == 1) checked @elseif($product->type == null) checked @endif>
                <label class="form-check-label" for="dropshipping">
                  {{ __("dropshipping") }}
                </label>
              </div>
              <div class="form-check">
                <input class="form-check-input @error('product_type') is-invalid @enderror" type="radio" name="product_type" id="affiliate" value="2" @if($product->type != null && $product->type == 2) checked @endif>
            ...
            <div class="mb-3">
              ...
              <input
                ...
                name="title"
                type="text"
                value="{{ old("title") ?? (isset($product->id) ? $product->title: "") }}"
            ...
            <div class="mb-3">
              ...
              <textarea
                ...
                name="description"
                placeholder="{{ __('product overview') }}">{{ old("description") ?? (isset($product->id) ? $product->description : "") }}</textarea>
            ...
            <div class="mb-3">
              ...
              <select id="cat" name="category" class="form-select @error('category') is-invalid @enderror">
                ...
                @foreach ($categories as $category)
                  <option @if(isset($product->id) && $category->id == $product->category_id) selected @endif value="{{ $category->id }}">{{ $category->title ?? "Unknown" }}</option>
                @endforeach
              </select>
              @error('category')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>
            <div class="row align-items-center" style="@if (isset($product->id) && $product->is_affiliate()) display: none @endif" id="price-section">
            ...
            <div class="col-12 mb-4">
              <div class="card shadow border-0 text-center p-0">
                <div class="card-body pb-5">
                  ...
                  <img src="{{ $product->main_image_url() }}" id="productImagePreview" class="rounded mx-auto m-0 article-thumbnail" alt="{{ __('product Image') }}">
                    ...
                    @if (isset($product->id) && $product->get_images() != null)
                      @foreach ($product->get_images() as $i => $img)
                        <div class="product-img rounded">
                          <input type="hidden" class="d-none" @if ($i == 0) name="old_images[0]" @else name="old_images[]" @endif value="{{ $img }}">
                          <img src="{{ $img }}" alt="{{ $img }}">
                          <div class="btns">
                            <button tabindex="-1" type="button" class="btn btn-sm btn-success"><i class="fa fa-image"></i></button>
                            <button tabindex="-1" type="button" class="btn btn-sm btn-danger"><i class="fa fa-trash"></i></button>
                          </div>
                        </div>
                      @endforeach
                    @endif
                  ...
                  <button type="button" onclick="document.getElementById('productImage').click()" class="btn btn-sm btn-warning d-inline-flex mx-3 align-items-center">
