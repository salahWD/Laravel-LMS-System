  ...
  <div class="container">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
      <div class="d-block mb-4 mb-md-0">
        <x-breadcrumb page="products" is-route=1 url="products_manage" />
        <h2 class="h4">{{ __('All :item', ["item" => __("Products")]) }}</h2>
        <p class="mb-0">{{ __('manage :page from one place.', ["page" => __('All :item', ["item" => __("Products")])]) }}</p>
      </div>
      <div class="btn-toolbar mb-2 mb-md-0">
        <a href="{{ route("product_create") }}" class="btn btn-sm btn-gray-800 d-inline-flex align-items-center">
          <svg class="icon icon-xs me-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg>
          {{ __('New :item', ["item" => __("Product")]) }}
        </a>
      </div>
    </div>
      ...
      <table class="table table-hover" id="products-table">
          ...
          <tbody>
            @foreach ($products as $product)
              <!-- Item -->
              <tr class="fw-normal product-row" id="product-{{ $product->id }}" data-id="{{ $product->id }}">
                <td class="p-1">
                  <a href="{{ route("product_edit", $product->id) }}">
                    <img class="ms-3 rounded-1 profile-pick" src="{{ $product->main_image_url() }}" alt="{{ __('product image') }}">
                  </a>
                </td>
                <td class="wrap">
                ...
                <td class="ps-5">
                  <a href="{{ route("product_edit", $product->id) }}">{{ $product->show_price() }}</a>
                </td>
                <td class="ps-5">
                  <a href="{{ route("product_edit", $product->id) }}">{{ $product->category?->title ?? "- - -" }}</a>
                </td>
                <td>
                  <div class="btn-group">
                    ...
                    <div class="dropdown-menu py-0">
                      ...
                      <hr class="my-0">
                      <button class="dropdown-item text-danger rounded-bottom delete-btn" id="product-{{ $product->id }}-delete" data-id="{{ $product->id }}">
                        <span class="fa me-2">
                          <svg class="me-2" data-slot="icon" fill="none" stroke-width="1.5" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
...
@section('scripts')
  <script>
    const page_id = {{ $products->currentPage() == 0 ? 1: $products->currentPage() }};
    const table = $("#products-table");

    $(".product-row .delete-btn").each(function () {
      $(this).on("click", function () {
        productDelete($(this))
      })
    })

    function productDelete(el) {
      ...
      Swal.fire({
        ...
        preConfirm: async (violation) => {
          try {
            ...
            const Url = `{{ url("app-request/products") }}/${ el.data("id") }/delete`;
        ...
        if (result.isConfirmed) {
          if (result.value.result) {
            $('#product-' + el.data("id")).fadeOut(300, function () {
              $(this).remove()
              if (result.value.next_product) {
                insertProduct(result.value.next_product)
              }
            })
          }
        }
    ...
    function insertProduct(data) {
      table.append(`
        <tr class="fw-normal product-row" id="product-${ data.id }" data-id="${ data.id }">
          <td class="p-1">
            <a href="{{ url(app()->getLocale() . "/dashboard/products") }}/${ data.id }/edit">
              <img class="ms-3 rounded-1 profile-pick" src="${ data.image }" alt="{{ __('product image') }}">
            </a>
          </td>
          <td class="wrap">
            <a href="{{ url(app()->getLocale() . "/dashboard/products") }}/${ data.id }/edit">${ data.title }</a>
          </td>
          <td class="ps-5">
            <a href="{{ url(app()->getLocale() . "/dashboard/products") }}/${ data.id }/edit">${ data.order }</a>
          </td>
          <td>
            <div class="btn-group">
              <button class="btn btn-link text-dark dropdown-toggle dropdown-toggle-split m-0 py-0 p-2" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
              ...
              <div class="dropdown-menu py-0">
                <a class="dropdown-item" href="{{ url(app()->getLocale() . "/dashboard/products") }}/${ data.id }/edit">
                  <span class="fa me-1">
                    <svg class="me-2" data-slot="icon" fill="none" stroke-width="1.5" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                      <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10"></path>
                    </svg>
                ...
                <hr class="my-0">
                <button class="dropdown-item text-danger rounded-bottom delete-btn" id="product-${ data.id }-delete" data-id="${ data.id }">
                  <span class="fa me-2">
                    <svg class="me-2" data-slot="icon" fill="none" stroke-width="1.5" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                      <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0"></path>
                    </svg>
      ...
      `)
      $(`#product-${ data.id }-delete`).on("click", function () {
        productDelete($(this))
      })
    }
