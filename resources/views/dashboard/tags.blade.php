              ...
              <tr class="fw-normal tag-row" id="tag-{{ $tag->id }}">
                ...
                <td>
                  <div class="btn-group">
                    ...
                    <a class="btn btn-sm btn-primary" href="{{ route("tag_view", $tag->id) }}">
                      <span class="fa w-22px">
                        <svg data-slot="icon" fill="none" stroke-width="1.5" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
...
@section("scripts")
  <script>
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          if (result.value.result) {
            $("{{ __('tag-') }}" + el.data("id")).fadeOut(300, function () {
              $(this).remove()
              if (result.value.next_tag) {
                inserttag(result.value.next_tag)
              }
            })
          }
        }
    ...
    function inserttag(data) {
      table.append(`
        <tr class="fw-normal tag-row" id="tag-${tag.id }" data-id="${ tag.id }">
          <td>${ tag.title }</td>
          <td class="wrap">${ tag.description }</td>
          <td>${ tag.slug }</td>
          <td>${ tag.articles_count }</td>
          <td>
            <div class="btn-group">
              <a class="btn btn-sm btn-info" href="{{ url("/dashboard/tags") }}/${ tag.id }">
                <span class="fa w-22px">
                  <svg data-slot="icon" fill="none" stroke-width="1.5" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                    <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10"></path>
                  </svg>
              ...
              <a class="btn btn-sm btn-primary" href="{{ url("/tags") }}/${ tag.id }">
                <span class="fa w-22px">
                  <svg data-slot="icon" fill="none" stroke-width="1.5" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z"></path>
                    <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"></path>
      ...
      `)
      $(`#tag-${ data.id }-delete`).on("click", function () {
        tagDelete($(this))
      })
    }
