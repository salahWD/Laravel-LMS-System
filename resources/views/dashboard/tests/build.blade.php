        ...
        <div class="col-4 order-2">
          <div class="sticky-top questions-sidebar mt-5">
            ...
            <div class="nav flex-column rounded py-1 gap-1 mb-2">
              ...
              <div class="shadow item mx-1 card flex-row align-items-center draggable-question overflow-hidden" draggable="true" data-item="{{ $type["id"] }}" data-name="{{ $type["title"] }}">
                  <div class="icon-bg py-1 px-3 d-flex bg-clr-secondary align-items-center justify-content-center" style="width: 60px;height: 45px;">
            ...
            <div class="nav flex-column rounded py-1 gap-1 mb-2">
              @foreach($result_types as $type)
                <div
                  class="shadow item mx-1 card flex-row align-items-center draggable-result overflow-hidden"
                  @if($type["id"] == 1 && $test->certificate != null) style="opacity: .25" draggable="false" @endif
                  draggable="true"
                  data-item="{{ $type["id"] }}"
                  data-name="{{ $type["title"] }}">
                  <div class="icon-bg py-1 px-3 d-flex bg-clr-info align-items-center justify-content-center" style="width: 60px;height: 45px;">
                    <i class="fa {{ $type["icon"] }}"></i>
                  </div>
                  <div class="card-body py-2 px-2 pe-3">
                    <h6 class="card-title my-1">{{ $type["title"] }}</h6>
          ...
          <div id="alerts" class="d-flex justify-content-end position-fixed" style="z-index:999;left: 60px;bottom: 180px;width: 400px;"></div>
        </div>
            ...
            <div class="modal-dialog modal-dialog-scrollable modal-xl">
              <div class="modal-content">
                <div class="modal-header px-4">
                  <h5 class="modal-title" id="modal_title" data-title="{{ __("Text Question") }}">{{ __("Text Question") }}</h5>
              ...
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <div class="mb-3">
                    ...
                    <input type="text" name="title" value="{{ $test->title }}" id="testTitle" class="form-control" placeholder="Test Title">
                    <div class="modal-footer">
                      <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ __("Close") }}</button>
                      <button type="button" class="btn btn-primary" data-bs-dismiss="modal" id="editNameSubmit">{{ __("Save changes") }}</button>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="alert shadow alert-primary p-4" style="border-style: dashed" id="introInfo">
            <form id="intro-form">
              <div class="form-check form-switch p-0 my-0 d-flex gap-3">
                ...
                <input class="form-check-input mx-0 float-none" name="has_intro" type="checkbox" role="switch" id="intro-switch" @if($test->has_intro()) checked @endif>
              </div>
              <div class="text-center" id="intro-holder">
                <div class="row">
                  <div class="col-md-7 d-flex flex-column align-items-center justify-content-center">
                    <h3 class="text-dark title my-3" data-target="introTitle-input" id="introTitle">{{ $test->intro_title }}</h3>
                    <input style="display: none" id="introTitle-input" type="text" name="intro_title" class="form-control mb-3" placeholder="intro title" value="{{ $test->intro_title }}">
                    <p class="text-dark lead mb-2" data-target="introDesc-input" id="introDesc">{{ $test->intro_description }}</p>
                    <textarea style="display: none" type="text" id="introDesc-input" name="intro_desc" class="form-control" placeholder="intro description">{{ $test->intro_description }}</textarea>
                    <button type="button"
                      class="btn btn-primary py-2 px-3 mt-3"
                      data-target="introButton-input" id="introButton">{{ $test->intro_btn }}</button>
                    <input style="display: none" id="introButton-input" name="intro_btn" type="text" class="form-control mb-3" placeholder="intro button" value="{{ $test->intro_btn }}">
                  </div>
                  <div class="col-md-5">
                    <button type="button" class="btn btn-secondary py-1 px-2 shadow mb-3" onclick="introImage.click();">
                      <i class="fa fas fa-solid fa-image"></i>
                    </button>
                    <button type="button" id="introImageRemove" class="btn btn-danger py-1 px-2 shadow mb-3 @if(!$test->has_intro_image()) d-none @endif">
                      <i class="fa fa-trash"></i>
                    </button>
                    <input type="file" name="intro_image" id="introImage" class="d-none" accept=".jpg,.jpeg,.png">
                    <div class="intro-image img-thumbnail @if(!$test->has_intro_image()) d-none @endif" id="previewImageIntro">
                      <img class="rounded" src="{{ $test->intro_image_url() }}" alt="intro image">
                    </div>
                  </div>
                </div>
              </div>
              <button type="button" id="save-intro" class="btn btn-info sender-btn">save <i class="fa fa-save"></i></button>
            </form>
          </div>
          <div id="contentItemsContainer" class="shadow alert bg-clr-secondary list-group questions mb-3 p-4" style="--bs-opacity:0.35; border-style: dashed" id="contentItemsContainer">
            ...
            <form id="result-form">
              <div id="result-holder">
                <div id="resultInvisiblity" style="transition: all 0.2s ease;">
                  <div class="row mb-2">
                    <div class="col-sm-6">
                      <label class="title text-dark" for="min_score">{{ __("minimum score") }}  ({{ __('total') }} {{ $test->totalScore() }}):</label>
                    </div>
                    <div class="col-sm-6">
                      <input value="{{ $test?->result?->min_score }}" type="number" max="{{ $test->totalScore() }}" name="min_score" id="min_score" class="form-control" placeholder="{{ __("minimum score to success") }}">
                    </div>
                  </div>
                  <div class="row mb-2">
                    <div class="col-sm-6">
                    ...
                    <div class="col-sm-6">
                      <input value="{{ $test?->result?->min_percent }}" type="number" min="0" name="min_percent" id="min_percent" class="form-control" placeholder="{{ __("minimum percentage to success") }}">
                    </div>
                  </div>
                  @if ($test->course_id == null)
                    <div class="row mb-2">
                      ...
                      <div class="col-sm-6">
                        <input value="{{ $test?->result?->max_attempts }}" type="number" min="1" name="max_attempts" id="max_attempts" class="form-control" placeholder="{{ __("maximum attempts allowed") }}">
                      </div>
                    </div>
                  @endif
                  <div class="row mb-2">
                    <div class="col-sm-6">
                      <label class="title text-dark" for="min_correct_questions">{{ __("minimum of (correctly answered questions)") }}:</label>
                    </div>
                    <div class="col-sm-6">
                      <input value="{{ $test?->result?->min_correct_questions }}" type="number" min="0" name="min_correct_questions" id="min_correct_questions" class="form-control" placeholder="{{ __("minimum (correct answered questions) to success") }}">
                    </div>
                  </div>
                  <div class="row mb-2">
                    <div class="col-sm-6">
                    ...
                    <div class="col-sm-6">
                      <textarea name="custom_note" id="custom_note" placeholder="{{ __("this note will appear on success") }}" class="form-control">{{ $test?->result?->note }}</textarea>
                    </div>
                  </div>
                </div>
                <button type="button" id="save-result" class="btn btn-info sender-btn">save <i class="fa fa-save"></i></button>
                <hr>
...
@section('scripts')
  ...
  <script>
    ...
    @if (count($test->questions) > 0)
      ...
      createQuestion('{{ $question->order }}', "{{ $question->title }}", '{{ $question->id }}', '{{ $question->type }}', "contentItemsContainer");
      @endforeach
    @endif
