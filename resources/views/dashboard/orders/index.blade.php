...
@section('meta')
  @php $page_title = config("app.name") . " | " . __('Orders') @endphp
@endsection
  ...
  <div class="container">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
      <div class="d-block mb-4 mb-md-0">
        <x-breadcrumb page="orders" is-route=1 url="orders_manage" />
        <h2 class="h4">{{ __('All Orders') }}</h2>
        <p class="mb-0">{{ __('manage all orders from one place.') }}</p>
      </div>
    </div>
      ...
      <div class="row align-items-center justify-content-between">
        <div class="col col-md-6 col-lg-3 col-xl-4">
          <div class="input-group me-2 me-lg-3 fmxw-400">
            ...
            <input type="text" class="form-control" placeholder="{{ __('Search Orders') }}">
          </div>
        </div>
        ...
        <tbody>
          @foreach ($orders as $order)
            ...
            <tr class="fw-normal">
              <td>
                <a
                  href="{{ route('order_edit', $order->id) }}">{{ $order->user != null ? $order->user->username : $order->client_name }}</a>
              </td>
              <td>
                <a href="{{ route('order_edit', $order->id) }}">{{ substr($order->address, 0, 55) }}</a>
              </td>
              <td>{{ $order->created_at }}</td>
              <td>
                <span class="fw-bold px-2 py-1 text-white rounded bg-{{ $order->status_class() }}">
                  {{ $order->status_title() }}
                </span>
              </td>
              <td>
                <div class="btn-group">
                  ...
                  <div class="dropdown-menu py-0">
                    <a class="dropdown-item" href="{{ route('order_edit', $order->id) }}">
                      <span class="fa me-1">
