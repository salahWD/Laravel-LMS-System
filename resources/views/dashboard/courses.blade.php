  ...
  <div class="container">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
      <div class="d-block mb-4 mb-md-0">
        <x-breadcrumb page="courses" is-route=1 url="courses_manage" />
        <h2 class="h4">{{ __('All Courses') }}</h2>
        <p class="mb-0">{{ __('manage all courses from one place.') }}</p>
      </div>
      ...
      <table class="table table-hover">
          <thead>
              <tr>
                ...
                <th class="border-gray-200">{{ __('items count') }}</th>
              ...
              <tr class="fw-normal">
                <td class="p-1">
                  <a href="{{ route("course_edit", $course->id) }}">
                    <img class="ms-3 rounded-1 profile-pick" src="{{ $course->image_url() }}" alt="{{ __('course image') }}">
                  </a>
                </td>
                <td class="wrap">
                  <a href="{{ route("course_edit", $course->id) }}">{{ $course->title }}</a>
                </td>
                <td>
                  <a href="{{ route("course_edit", $course->id) }}">{{ $course->show_price() }}</a>
                </td>
                <td>
                  {{ $course->items_count }}
                </td>
                <td>
                  <div class="btn-group">
                    ...
                    <div class="dropdown-menu py-0">
                      <a class="dropdown-item" href="{{ route("course_edit", $course->id) }}">
                        <span class="fa me-1">
                      ...
                      <hr class="my-0">
                      <a class="dropdown-item text-danger rounded-bottom" href="{{ route("course_delete", $course->id) }}">
                        <span class="fa me-2">
                          <svg class="me-2" data-slot="icon" fill="none" stroke-width="1.5" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
