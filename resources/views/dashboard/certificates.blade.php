  ...
  <div class="container">
    ...
    <div class="fixed-bottom p-4">
      <div class="position-relative">
        @if (session()->has('certificate_added') && session()->get('certificate_added'))
          <div id="liveToast" class="toast align-items-center text-white bg-primary border-0 position-absolute"
            ...
            <div class="d-flex">
              <div class="toast-body">
                {{ __('Certificate Added Successfully') }}.
              </div>
        ...
        @endif
        @if (session()->has('certificate_edited') && session()->get('certificate_edited'))
          <div id="liveToast" class="toast align-items-center text-white bg-primary border-0 position-absolute"
            role="alert" aria-live="assertive" aria-atomic="true"
            style="top: -120px;{{ app()->getLocale() == 'ar' ? 'left' : 'right' }}: 0;">
            <div class="d-flex">
              <div class="toast-body">
                {{ __('Certificate Edited Successfully') }}.
              </div>
              <button type="button" onclick="$('#liveToast').fadeOut(450)" class="btn-close btn-close-white me-2 m-auto"
                data-bs-dismiss="toast" aria-label="Close"></button>
            </div>
    ...
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
      <div class="d-block mb-4 mb-md-0">
        <x-breadcrumb page="certificates" is-route=1 url="certificates_manage" />
        <h2 class="h4">{{ __('All Certificates') }}</h2>
        <p class="mb-0">{{ __('manage all certificates from one place') }}.</p>
      </div>
      <div class="btn-toolbar mb-2 mb-md-0">
        <a href="{{ route('certificate_create') }}" class="btn btn-sm btn-gray-800 d-inline-flex align-items-center">
      ...
      <div class="row align-items-center justify-content-between">
        <div class="col col-md-6 col-lg-3 col-xl-4">
          <div class="input-group me-2 me-lg-3 fmxw-400">
            ...
            <input type="text" class="form-control" placeholder="{{ __('Search Courses') }}">
          </div>
        </div>
      ...
      <table class="table table-hover">
        ...
        <tbody>
          @foreach ($certificates as $certificate)
            ...
            <tr class="fw-normal">
              <td class="wrap">
                <a href="{{ route('certificate_edit', $certificate->id) }}">{{ $certificate->title }}</a>
              </td>
              <td>
                <a href="{{ route('certificate_edit', $certificate->id) }}">{!! $certificate->show_status() !!}</a>
              </td>
              <td class="ps-5">
                <a href="{{ route('certificate_edit', $certificate->id) }}">{{ $certificate->show_price() }}</a>
              </td>
              <td>
                <div class="btn-group">
                  ...
                  <div class="dropdown-menu py-0">
                    <a class="dropdown-item" href="{{ route('certificate_edit', $certificate->id) }}">
                      <span class="fa me-1">
...
@if (
    (session()->has('certificate_added') && session()->get('certificate_added')) ||
        (session()->has('certificate_edited') && session()->get('certificate_edited')))
  @php session()->forget('certificate_added') @endphp
  @section('scripts')
    <script>
