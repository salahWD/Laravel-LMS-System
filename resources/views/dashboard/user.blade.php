            ...
            <div class="card shadow border-0 text-center p-0">
              <div class="card-body pb-5">
                ...
                <button class="btn btn-sm btn-danger d-inline-flex align-items-center" data-bs-toggle="tooltip"
                  data-bs-placement="top" title="{{ __('Delete User') }}" id="delete-user">
                ...
                <button
                  class="btn btn-sm d-inline-flex mx-3 align-items-center {{ $user->permission == 0 ? 'btn-success text-white' : 'btn-warning' }}"
                  data-bs-toggle="tooltip" data-bs-placement="top" data-action="{{ $user->permission == 0 }}"
                  title="{{ $user->permission == 0 ? __('Enable / Activate User') : __('Disable / Inactivate User') }}"
                  id="disable-user">
...
@section('scripts')
  <script>
      ...
      Swal.fire({
        ...
        preConfirm: async (input) => {
          try {
            ...
            const response = await fetch("{{ route('user_delete_api', $user->id) }}", {
              method: "POST",
              body: data,
              headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        if (result.isConfirmed) {
          Swal.fire({
            ...
            confirmButtonText: '<a href="{{ route('users_manage') }}">Continue</a>',
            buttonsStyling: false,
          });
        }
      });
    ...
    document.getElementById("disable-user").addEventListener('click', function() {
      Swal.fire({
        title: "Are You Sure ?",
        customClass: {
          confirmButton: this.dataset.action == 1 ? 'text-white btn btn-success me-4' : 'btn btn-warning me-4',
          cancelButton: 'btn btn-primary'
        },
        buttonsStyling: false,
        inputAttributes: {
        ...
        showCancelButton: true,
        confirmButtonText: this.dataset.action == 1 ? "{{ __('Enable User') }}" : "{{ __('Disable User') }}",
        showLoaderOnConfirm: false,
        preConfirm: async (input) => {
          try {
            ...
            const response = await fetch(this.dataset.action == 1 ? "{{ route('user_edit', $user->id) }}/e" :
              "{{ route('user_edit', $user->id) }}/d", {
                method: "POST",
                body: data,
                headers: {
                  'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        if (result.isConfirmed) {
          Swal.fire({
            title: this.dataset.action == 1 ? `{{ _('User Enabled Successfully') }}` :
              `{{ _('User Disabled Successfully') }}`,
            customClass: {
              confirmButton: 'text-white btn btn-success me-4',
              cancelButton: 'btn btn-primary'
            },
            ...
            if (this.dataset.action == 1) {
              badge.classList.remove("bg-warning", "text-black");
              badge.classList.add("bg-success");
              badge.innerText = '{{ __('Active') }}';
