    ...
    <div class="fixed-bottom p-4">
      <div class="position-relative">
        @if(session()->has('course_enrolled') && session()->get('course_enrolled'))
          <div id="liveToast" class="toast align-items-center text-white bg-primary border-0 position-absolute" role="alert" aria-live="assertive" aria-atomic="true" style="bottom: 0;{{ app()->getLocale() == "ar" ? "left": "right"}}: 0;">
            <div class="d-flex">
        ...
        @foreach($items as $item)
          <div class="col-md-4 mb-3 lecture">
            @if ($item->is_open && $is_enrolled)
              <a href="@if ($item->is_lecture()) {{ route("lecture_show", $item->itemable->id) }} @else {{ route("test_show", $item->itemable->id) }} @endif">
                <div style="border-width: 2px;" class="card bg-dark text-white @if($item->is_watched) border-success @else border-primary @endif">
                  <img class="card-img lecture-img" src="{{ $item->itemable->image_url() }}" alt="{{ $item->itemable->title }}">
            ...
            @else
              <div style="border-width: 2px;" class="card bg-dark text-white position-relative @if($item->is_watched) border-success @else border-0 @endif">
                <span class="lock-icon">
                  <i class="fa fa-lock"></i>
                </span>
                <img class="card-img lecture-img closed-lecture" src="{{ $item->itemable->image_url() }}" alt="{{ $item->itemable->title }}">
                <div class="card-img-overlay">
                  <h5 class="card-title m-0">{{ $item->itemable->title }}</h5>
                  <small class="card-text text-sm">{{ $item->itemable->get_date() }}</small>
                </div>
