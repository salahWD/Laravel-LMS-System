...
class CategoryController extends Controller {
  ...
  public function show(Category $category) {
    $articles = $category->notProduct()->with("articles")->join("articles", "articles.category_id", "=", "categories.id")->where("articles.status", 2)->translatedIn(app()->getLocale())->paginate(config("settings.tables_row_count"));
